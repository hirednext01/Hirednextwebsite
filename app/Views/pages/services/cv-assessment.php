        ...
        <div class="grid md:grid-cols-2 gap-8 items-stretch">
            <article class="bg-white rounded-[2rem] border border-gray-200 p-8 md:p-10 shadow-sm">
                ...
                <ul class="space-y-4 text-sm text-gray-700 mb-8">
                    <li>✓ CV readability and structure review</li>
                    <li>✓ Key strengths and obvious gaps</li>
                    <li>✓ Basic ATS-readiness observations</li>
                    <li>✓ Practical improvement pointers</li>
            ...
            <article class="bg-primary text-white rounded-[2rem] border-2 border-accent p-8 md:p-10 shadow-xl relative overflow-hidden">
                ...
                <ul class="space-y-4 text-sm text-white/85 mb-8">
                    <li>✓ Everything in the free assessment</li>
                    <li>✓ Role-specific CV fit review</li>
                    <li>✓ ATS and keyword gap analysis</li>
                    <li>✓ Experience positioning recommendations</li>
                    <li>✓ Priority delivery within 12 hours</li>
                </ul>
...
<section id="assessment-form" class="py-20 bg-white">
    <div class="max-w-[900px] mx-auto px-4 sm:px-8 lg:px-12">
        <div class="text-center mb-12">
            <h2 class="text-4xl font-serif font-bold text-primary mb-4">Start your CV assessment</h2>
            <p class="text-gray-600">Submit your details and CV. We'll use your selected service and job context to process the request.</p>
        </div>
        <form action="<?= base_url('api/contact/submit') ?>" method="post" enctype="multipart/form-data" class="bg-gray-50 border border-gray-200 rounded-[2rem] p-8 md:p-10 space-y-5">
            <input type="hidden" name="service" value="CV Assessment">
            <input type="hidden" name="job_slug" value="<?= esc($job['slug'] ?? '') ?>">
            <input type="hidden" name="job_title" value="<?= esc($job['title'] ?? '') ?>">
            <div class="grid md:grid-cols-2 gap-5">
                <input name="name" required placeholder="Full name" class="w-full border border-gray-200 rounded-xl px-4 py-3 bg-white">
                <input name="email" type="email" required placeholder="Email address" class="w-full border border-gray-200 rounded-xl px-4 py-3 bg-white">
            </div>
            <input name="phone" required placeholder="Phone number" class="w-full border border-gray-200 rounded-xl px-4 py-3 bg-white">
            <select name="assessment_plan" required class="w-full border border-gray-200 rounded-xl px-4 py-3 bg-white">
                <option value="">Select assessment</option>
                <option value="free">Free CV Assessment — 7–10 days</option>
                <option value="priority_599">Priority CV Assessment — ₹599 / 12 hours</option>
            </select>
            ...
            <div class="rounded-xl border border-dashed border-gray-300 bg-white px-4 py-4">
                <label class="block text-sm font-bold text-primary mb-2">Upload your CV</label>
                <input name="resume" type="file" accept=".pdf,.doc,.docx" required class="w-full text-sm">
                <p class="text-xs text-gray-500 mt-2">PDF, DOC or DOCX. Maximum 5MB.</p>
            </div>
            ...
            <p class="text-xs text-gray-500 text-center">Your assessment request is separate from any job application.</p>
        </form>
    </div>
</section>
...
<section class="py-16 bg-gray-50">
    <div class="max-w-[900px] mx-auto px-4 sm:px-8 lg:px-12">
        <h2 class="text-3xl font-serif font-bold text-primary text-center mb-10">CV Assessment FAQs</h2>
        <div class="space-y-4">
            <details class="bg-white border border-gray-200 rounded-2xl p-5"><summary class="font-bold text-primary cursor-pointer">What does the free CV assessment include?</summary><p class="mt-3 text-gray-600 text-sm leading-relaxed">It covers structure, readability, key strengths, obvious gaps and basic ATS-readiness observations, with delivery within 7–10 days.</p></details>
            <details class="bg-white border border-gray-200 rounded-2xl p-5"><summary class="font-bold text-primary cursor-pointer">What is different about the ₹599 assessment?</summary><p class="mt-3 text-gray-600 text-sm leading-relaxed">The priority assessment adds role-specific fit, ATS and keyword gaps, experience-positioning recommendations and priority delivery within 12 hours.</p></details>
            <details class="bg-white border border-gray-200 rounded-2xl p-5"><summary class="font-bold text-primary cursor-pointer">Does a CV assessment apply for a job automatically?</summary><p class="mt-3 text-gray-600 text-sm leading-relaxed">No. A CV assessment is a separate career service. If you want to apply for a HiredNext vacancy, use the Apply button on that specific job page.</p></details>
        </div>
    </div>
</section>
...
<?= json_encode([
    ...
    '@type' => 'Service',
    'name' => 'HiredNext CV Assessment',
    'provider' => ['@type' => 'Organization', 'name' => 'HiredNext Recruitment', 'url' => base_url()],
    'serviceType' => 'CV Assessment',
    'offers' => [
        ['@type' => 'Offer', 'price' => '0', 'priceCurrency' => 'INR', 'name' => 'Free CV Assessment'],
        ['@type' => 'Offer', 'price' => '599', 'priceCurrency' => 'INR', 'name' => 'Priority CV Assessment'],
    ],
