...
$proofHref = $linkedinUrl !== '' ? $linkedinUrl : $sourceUrl;
...
<article class="testimonial-luxe-card group relative overflow-hidden rounded-[1.75rem] border <?= $cardClasses ?> p-7 md:p-9 transition-all duration-500 hover:-translate-y-1">
    ...
    <div class="relative z-10 h-full flex flex-col">
        <div class="flex items-start justify-between gap-5 mb-7">
            <div class="flex flex-wrap items-center gap-2">
                <span class="px-3 py-1.5 rounded-full border <?= $isDark ? 'border-white/15 bg-white/5 text-white/65' : 'border-primary/10 bg-primary/[0.035] text-primary/65' ?> text-[9px] uppercase tracking-[0.18em] font-black">
                    <?= esc($isCandidate ? 'Placed candidate' : $proofType) ?>
                </span>
                <?php if ($proofHref !== ''): ?>
                    <span class="px-3 py-1.5 rounded-full border <?= $isDark ? 'border-gold/25 bg-gold/10 text-gold' : 'border-[#e8dcc0] bg-[#fbf6e9] text-[#8b6d24]' ?> text-[9px] uppercase tracking-[0.18em] font-black">Identity / source linked</span>
                ...
                <?php endif; ?>
                <?php if ($rating > 0): ?><span class="text-gold text-xs ml-1" aria-label="<?= esc((string)$rating) ?> out of 5 rating"><?php for ($i=1;$i<=5;$i++): ?><?= $i <= $rating ? '★' : '☆' ?><?php endfor; ?></span><?php endif; ?>
            </div>
        ...
        <blockquote class="text-[16px] md:text-[17px] <?= $mutedClasses ?> leading-[1.8] mb-8"><?= esc($quote) ?></blockquote>
        ...
        <div class="mt-auto pt-6 border-t <?= $borderClasses ?> flex flex-col sm:flex-row sm:items-end sm:justify-between gap-5">
            ...
            <?php if ($proofHref !== ''): ?>
                <div class="shrink-0"><a href="<?= esc($proofHref) ?>" target="_blank" rel="noopener noreferrer external" class="inline-flex items-center gap-2 text-xs font-extrabold <?= $isDark ? 'text-gold hover:text-white' : 'text-accent hover:text-primary' ?> transition-colors" aria-label="View evidence for <?= esc($name) ?>"><?= esc($proofLabel) ?> <span aria-hidden="true">↗</span></a></div>
            <?php endif; ?>
        </div>
    </div>
</article>
