...
class SeedAuthorityBlogs extends BaseCommand
{
    protected $group = 'SEO';
    protected $name = 'blog:seed-authority';
    protected $description = 'Insert HiredNext cornerstone SEO/AEO/GEO authority blogs without changing or overwriting existing posts.';
    protected $usage = 'blog:seed-authority [--dry-run]';
    ...
    public function run(array $params)
    {
        ...
        $config = config('AuthorityBlogs');
        if (!$config || empty($config->posts) || !is_array($config->posts)) {
            CLI::error('Authority blog configuration is empty. No changes made.');
            return;
        }
        ...
        foreach ($config->posts as $post) {
            ...
            $existing = $db->table('blog_posts')->select('id, title, slug')->where('slug', $slug)->get()->getRowArray();
            if ($existing) {
            ...
            $ok = $db->table('blog_posts')->insert($insert);
            if (!$ok) {
        ...
        CLI::newLine();
        CLI::write('Authority posts configured: ' . count($config->posts), 'yellow');
