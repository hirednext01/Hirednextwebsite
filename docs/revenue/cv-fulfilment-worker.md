...
New website alerts also use the existing bridge's `Website CV Capture` trigger at the fixed `/webhook/hirednext-cv-capture` path. Authentication is the same expiring order-bound capability in its POST body: the website validates it before reserving any email. The original header-authenticated `CV Order Request` trigger is unchanged. The relay uses the verified existing jobs Gmail credential, sends only to jobs, records the real Gmail ID through `capture_receipt`, and returns a minimal captured status. Replay cannot emit another alert. Neither capture nor a receipt for this email confirms a payment. The legacy alert is retained as a fallback and may be a duplicate; dedupe by canonical order/reference, not message count. Do not launch historical orders through this relay.
...
1. `inspect` returns the canonical order and fulfilment state. Check service, amount, recipient and reference against the alert. This document covers Priority CV Assessment. New `rebuild_1799` orders use `cv-rebuild-worker.md` through the same bridge and owner. Preserve existing candidate-specific and other-service owners.
